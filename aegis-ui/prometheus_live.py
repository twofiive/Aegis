...
class PrometheusLiveClient:
    ...
    def get_node_metrics(self, node_name):
        ...
        print(f"🔍 [Prometheus] Recherche des métriques pour le nœud : {node_name}")
        ...
        for name, query in self.queries.items():
            try:
                ...
                if not found and results:
                    instances_dispos = [res['metric'].get('instance') for res in results]
                    print(f"   ⚠️ Non trouvé pour {name}. Instances dispos dans Prometheus : {instances_dispos}")
...
if __name__ == "__main__":
    ...
    print(json.dumps(resultat_dict, indent=4))